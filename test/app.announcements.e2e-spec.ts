...
describe('Integration Tests: Announcements Routes', () => {
  ...
  describe('GET ---> /announcements', () => {
    ...
    it('Should not be able get all announcements with invalid advertiser id', async () => {
      const { status, body } = await request(app.getHttpServer()).get(
        `/announcements/advertiser/${'invalidId'}`,
      );
      expect(status).toBe(404);
      expect(body).toStrictEqual(mockedErrorResponse);
    });
    ...
    it('Should be able get all announcements with valid advertiser id', async () => {
      const { status, body } = await request(app.getHttpServer()).get(
        `/announcements/advertiser/${advertiserId}`,
      );
      expect(status).toBe(200);
      expect(body[0]).toHaveProperty('id');
      expect(body[0]).toHaveProperty('title');
